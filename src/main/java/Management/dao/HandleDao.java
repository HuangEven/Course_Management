...
public class HandleDao {
    ...
    public void handleTeamShare(BigInteger id,Integer status){
        ...
        if(DataUtil.isNotEmpty(shareTeamApplication)){
            ...
            List<Team> teams=teamMapper.queryByCourseId(mainCourseId);
            ...
            if(status==1) {
                ...
                for(int i=0;i<teams.size();i++){
                    this.teamChangeCourse(teams.get(i),mainCourse,subCourse);
                }
            }
            if(status==0){
                mainCourseId = shareTeamApplication.getMainCourseId();
    ...
    private void teamChangeCourse(Team team,Course mainCourse,Course subCourse){
        ...
        for(int i=0;i<studentList.size();i++){
            ...
            Long klassId=null;
            //获得这个学生的klassId
            if (classDao.getKlassInfo(student.getId(),subCourse.getId())!=null)
             klassId=classDao.getKlassInfo(student.getId(),subCourse.getId()).getId().longValue();
            //收集每个学生的klassId
            if(klassId!=null){
        ...
        //创建新的klass_team的副本
        if (maxKlassId!=null)
             klassTeamMapper.insertShareTeam(maxKlassId,team.getId().longValue());
    }
}
